...
2. **ID uniqueness.** No ID-typed attribute value (local name `ID`/`Id`/`xml:id`) may be shared by two elements; the consumed assertion's `ID` is itself an ID-typed value, so this guarantees the backend cannot resolve that ID to a different element. Only ID-typed attributes are compared — a non-ID attribute (for example a `Name`) whose value coincides with the ID cannot influence ID resolution, so comparing it would only risk a false rejection. The uniqueness set is a verified superset of what the backend actually resolves: pysaml2 7.5.0 drives xmlsec1 with `--id-attr:ID ...:Assertion` + `--node-id <assertion ID>` on the signature-verification path (and `--id-attr:Id` on the unrelated decrypt path), so covering `ID`/`Id`/`xml:id` spans every attribute the backend can treat as the resolvable ID; DTD-declared IDs are foreclosed by rejecting any `<!DOCTYPE>` at the parse boundary. Treating those names document-wide is deliberate fail-closed defense-in-depth: xmlsec1 scopes `ID` to `<Assertion>` elements today (so a non-Assertion `ID` is inert) and libxml2 honors `xml:id` document-wide (a colliding `xml:id` already fails verification closed), but rejecting any duplicate ID-typed value keeps the invariant independent of the backend's version-specific ID registration — a document reusing an ID-typed value is anomalous (`xs:ID` requires uniqueness) regardless of whether this backend would be confused by it, and coupling the check to xmlsec1 internals is exactly what this hardening avoids. Comparisons apply XML whitespace normalization (`xml:id`/`xs:ID` collapse whitespace) so a padded value cannot evade a match the backend would resolve as equal. Bare lowercase `id` is excluded (not ID-typed without a DTD/schema; appears in XHTML `AttributeValue` content).
